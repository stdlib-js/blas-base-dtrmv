...
function dtrmv( uplo, trans, diag, N, A, strideA1, strideA2, offsetA, x, strideX, offsetX ) { // eslint-disable-line max-params, max-len
	...
	if ( u === null ) {
		throw new TypeError( format( '1yfGT', uplo ) );
	}
	t = resolveTranspose( trans );
	if ( t === null ) {
		throw new TypeError( format( '1yfGG', trans ) );
	}
	d = resolveDiagonal( diag );
	if ( d === null ) {
		throw new TypeError( format( '1yfGU', diag ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '1yfGH', N ) );
	}
	if ( strideA1 === 0 ) {
		throw new RangeError( format( '1yfGB', strideA1 ) );
	}
	if ( strideA2 === 0 ) {
		throw new RangeError( format( '1yfG2', strideA2 ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '1yfG3', strideX ) );
	}
	if ( N === 0 ) {
		return x;
	}
