...
function dtrmv( order, uplo, trans, diag, N, A, LDA, x, strideX ) {
	...
	if ( l === null ) {
		throw new TypeError( format( '1yfFx', order ) );
	}
	u = resolveTriangle( uplo );
	if ( u === null ) {
		throw new TypeError( format( '1yfFy', uplo ) );
	}
	t = resolveTranspose( trans );
	if ( t === null ) {
		throw new TypeError( format( '1yfGO', trans ) );
	}
	d = resolveDiagonal( diag );
	if ( d === null ) {
		throw new TypeError( format( '1yfGP', diag ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '1yfGQ', N ) );
	}
	if ( LDA < max( 1, N ) ) {
		throw new RangeError( format( '1yfGR', N, LDA ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '1yfGS', strideX ) );
	}
	if ( N === 0 ) {
		return x;
	}
