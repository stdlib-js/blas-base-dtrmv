...
tape( 'the function has an arity of 9', function test( t ) {
	t.strictEqual( dtrmv.length, 9, 'returns expected value' );
	t.end();
});
...
tape( 'the function throws an error if provided an invalid first argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dtrmv( value, data.uplo, data.trans, data.diag, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid second argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dtrmv( data.order, value, data.trans, data.diag, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid third argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dtrmv( data.order, data.uplo, value, data.diag, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid fourth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dtrmv( data.order, data.uplo, data.trans, value, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid fifth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dtrmv( data.order, data.uplo, data.trans, data.diag, value, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid seventh argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dtrmv( data.order, data.uplo, data.trans, data.diag, data.N, new Float64Array( data.A ), value, new Float64Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid ninth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dtrmv( data.order, data.uplo, data.trans, data.diag, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), value );
		};
	}
});
...
tape( 'the function performs one of the matrix-vector operations `x = A*x` or `x = A^T*x` (row-major, lower, no-transpose, non-unit)', function test( t ) {
	...
	out = dtrmv( data.order, data.uplo, data.trans, data.diag, data.N, a, data.LDA, x, data.strideX );
...
tape( 'if `N` is zero, the function returns the input vector unchanged (row-major)', function test( t ) {
	...
	out = dtrmv( data.order, data.uplo, data.trans, data.diag, 0, a, data.LDA, x, data.strideX );
	t.strictEqual( out, x, 'returns expected value' );
